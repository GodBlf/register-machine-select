...
func NewHandler(cfg config.AppConfig, manager *manager.Manager, scanner *scanner.Service, fileops *fileops.Service) *Handler {
	return &Handler{
		cfg:     cfg,
		manager: manager,
		scanner: scanner,
...
func (h *Handler) apiStatus(c *gin.Context) {
	running, hasResult := h.manager.Status()
	c.JSON(http.StatusOK, gin.H{
		"running":    running,
