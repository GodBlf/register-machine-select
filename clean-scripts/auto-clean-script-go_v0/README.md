...
- `scan.schedule_interval_seconds`
  定时扫描间隔，单位秒。`0` 表示关闭定时扫描；大于 `0` 时服务启动后会按该周期自动发起一次默认扫描清理。定时扫描使用服务启动时从 `config.yaml` 读取到的默认配置。
...
- `scan.delete_401`
  扫描完成后是否自动删除首轮扫描出的 `401` 文件。该开关同时作用于手动扫描和定时扫描。
...
8. 如果启用 `delete_401`：
   - 删除首轮扫描得到的 `401` 文件
9. 如果配置了 `scan.schedule_interval_seconds > 0`：
   - 服务会周期性重复上述流程
   - 若某一时刻已有手动或定时扫描在运行，则该周期会跳过，不会并发启动第二个扫描
   - 如果同时配置 `delete_401: true`，定时扫描完成后也会自动删除本轮识别出的 `401` 文件
   - 如果同时配置 `no_quarantine: false`，定时扫描也会执行超限文件移入 `exceeded_dir` 和恢复回扫
...
```json
{
  "running": false,
  "has_result": true,
  "last_result_at": "2026-03-24T10:15:00.0000000+08:00",
  "schedule": {
    "enabled": true,
    "interval_seconds": 300,
    "next_run_at": "2026-03-24T10:20:00.0000000+08:00",
    "last_triggered_at": "2026-03-24T10:15:00.0000000+08:00",
...
- 修改本次扫描参数
- 查看 `config.yaml` 中的定时扫描间隔、下次执行时间和最近一次调度状态
- 查看实时进度条
- 查看统计卡片
...
除定时扫描间隔外，前端参数只覆盖当前请求，不会回写 `config.yaml`。定时间隔在页面中只读展示，实际值来自服务启动时加载的 `config.yaml`。

如果你修改了 `schedule_interval_seconds`、`delete_401`、`no_quarantine` 等默认扫描配置，需要重启服务后定时任务才会使用新值。
...
- 定时扫描与手动扫描共用同一套扫描逻辑；因此 `delete_401=true` 会让定时扫描也自动删除 `401` 文件，`no_quarantine=false` 会让定时扫描也移动超限文件。
