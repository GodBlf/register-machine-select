...
class SelfHostedMailApiProvider(MailProviderBase):
    provider_name = "self_hosted_mail_api"

    def __init__(self, *, proxy: str, logger: logging.Logger, api_base: str, api_key: str, domain: str):
    ...
    def create_mailbox(self) -> Optional[Mailbox]:
        email = f"oc{secrets.token_hex(5)}@{self.domain}"
    ...
    def describe(self) -> str:
        return f"{self.provider_name}({self.api_base}, domain={self.domain})"


class DuckMailProvider(MailProviderBase):
    provider_name = "duckmail"

    def __init__(self, *, proxy: str, logger: logging.Logger, api_base: str, bearer: str, domain: str = "duckmail.sbs"):
        super().__init__(proxy=proxy, logger=logger)
        self.api_base = str(api_base or "https://api.duckmail.sbs").rstrip("/")
        self.bearer = str(bearer or "").strip()
        self.domain = normalize_mail_domain(domain or "duckmail.sbs")
        if not self.bearer:
    ...
    def create_mailbox(self) -> Optional[Mailbox]:
        ...
        return Mailbox(email=email, password=password, token=token)
...
class YYDSMailProvider(MailProviderBase):
    ...
    def create_mailbox(self) -> Optional[Mailbox]:
        ...
        return Mailbox(email=email, token=token, account_id=account_id)
    ...
    def _fetch_message_detail(self, token: str, message_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            resp = session.get(
                f"{self.api_base}/messages/{quote(str(message_id), safe='')}",
                headers=self._temp_headers(token),
                timeout=30,
                verify=False,
            )
...
class ProtocolRegistrar:
    def __init__(self, proxy: str, logger: logging.Logger, conf: Optional[Dict[str, Any]] = None):
        ...
        self.entry_mode = parse_choice(
            pick_conf(self.conf, "registration", "entry_mode", default="direct_auth"),
    ...
    def _init_session_via_chatgpt_web(self) -> tuple[bool, str]:
        chatgpt_base = str(pick_conf(self.conf, "registration", "chatgpt_base", default="https://chatgpt.com") or "https://chatgpt.com").rstrip("/")
...
class RegisterRuntime:
    ...
    def wait_for_provider_availability(self, worker_id: int = 0) -> None:
        while True:
            ...
            with self.health_lock:
                cooldown_until = self.provider_cooldown_until
                consecutive_failures = self.provider_consecutive_failures
            now = time.time()
            if cooldown_until <= now:
                return

            wait_seconds = max(0.0, cooldown_until - now)
            self.logger.warning(
                "共享链路处于冷却期，worker=%s 等待 %.1fs 后重试（连续失败=%s）",
                worker_id or "-",
                wait_seconds,
    ...
    def note_attempt_failure(self, stage: str, email: str = "", detail: str = "") -> None:
        threshold = max(1, self.failure_threshold_for_cooldown)
        cooldown_seconds = max(1.0, self.failure_cooldown_seconds)
        normalized_stage = str(stage or "unknown").strip() or "unknown"
        normalized_detail = str(detail or "").strip()
        with self.health_lock:
            self.provider_consecutive_failures += 1
            failure_count = self.provider_consecutive_failures
            should_cooldown = failure_count >= threshold
            if should_cooldown:
                self.provider_cooldown_until = max(self.provider_cooldown_until, time.time() + cooldown_seconds)
            cooldown_until = self.provider_cooldown_until
        with self.stats_lock:
        ...
        if should_cooldown:
            self.logger.warning(
                "失败归类: stage=%s detail=%s email=%s consecutive=%s/%s | 进入冷却至 %s",
                normalized_stage,
            ...
            self.logger.warning(
                "失败归类: stage=%s detail=%s email=%s consecutive=%s/%s",
                normalized_stage,
                normalized_detail or "-",
                email or "-",
                failure_count,
...
async def run_probe_async(
    ...
    return probe_results, len(files), len(candidates), files
...
async def run_clean_401_async(
    *,
    ...
    probe_results, total_files, codex_files, files = await run_probe_async(
        base_url=base_url,
        token=token,
    ...
    logger.info(
        "探测完成: 总账号=%s, codex账号=%s, 401失效=%s, used_percent超标=%s, quota=%s, healthy+disabled=%s",
        total_files,
        codex_files,
        invalid_401_count,
...
def run_clean_401(conf: Dict[str, Any], logger: logging.Logger) -> Dict[str, Any]:
    ...
    logger.info("开始清理账号: base_url=%s target_type=%s used_percent_threshold=%s",
                base_url, target_type, used_percent_threshold)
    return asyncio.run(
        run_clean_401_async(
            base_url=base_url,
            token=token,
