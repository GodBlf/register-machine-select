...
def _load_config():
    ...
    if os.path.exists(config_path):
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                ...
                legacy_base = file_config.get("yydsmail_api_base")
                if not file_config.get("tempmail_lol_api_base") and isinstance(legacy_base, str) and "tempmail.lol" in legacy_base:
                    config["tempmail_lol_api_base"] = legacy_base
    ...
    legacy_base_env = os.environ.get("YYDSMAIL_API_BASE")
    if "TEMPMAIL_LOL_API_BASE" not in os.environ and legacy_base_env and "tempmail.lol" in legacy_base_env:
        config["tempmail_lol_api_base"] = legacy_base_env
...
TEMPMAIL_LOL_API_BASE = _CONFIG["tempmail_lol_api_base"].rstrip("/")
DEFAULT_TEMPMAIL_USER_AGENT = (
...
def _create_tempmail_lol_session(user_agent: str = DEFAULT_TEMPMAIL_USER_AGENT, proxy: str = None):
    """创建 tempmail.lol 请求会话"""
    session = curl_requests.Session()
    session.headers.update({
        "User-Agent": user_agent or DEFAULT_TEMPMAIL_USER_AGENT,
...
def _create_tempmail_lol_inbox(session, impersonate: str = None):
    """创建 tempmail.lol 临时邮箱，返回 (email, mail_token)"""
    try:
        request_kwargs = {"json": {}, "timeout": 15}
        if impersonate:
            request_kwargs["impersonate"] = impersonate
        res = session.post(f"{TEMPMAIL_LOL_API_BASE}/inbox/create", **request_kwargs)
        ...
        if not isinstance(payload, dict):
            raise Exception(f"创建邮箱返回格式异常: {payload!r}")

        email = str(payload.get("address") or "").strip()
        mail_token = str(payload.get("token") or "").strip()
        if not email or not mail_token:
            raise Exception(f"创建邮箱成功但缺少 address/token, 响应: {payload}")

        return email, mail_token

    except Exception as e:
        raise Exception(f"tempmail.lol 创建邮箱失败: {e}")


def create_temp_email():
    """创建 tempmail.lol 临时邮箱，返回 (email, mail_token)"""
    session = _create_tempmail_lol_session()
    return _create_tempmail_lol_inbox(session, impersonate="chrome131")


def _request_tempmail_lol_inbox(session, mail_token: str, impersonate: str = None):
    """从 tempmail.lol 获取 inbox 原始响应"""
    try:
        request_kwargs = {"timeout": 15}
        if impersonate:
            request_kwargs["impersonate"] = impersonate
        res = session.get(
            f"{TEMPMAIL_LOL_API_BASE}/inbox?{urlencode({'token': mail_token})}",
            **request_kwargs,
        )
...
def _fetch_tempmail_lol_inbox(mail_token: str):
    """从 tempmail.lol 获取 inbox 原始响应"""
    session = _create_tempmail_lol_session()
    return _request_tempmail_lol_inbox(session, mail_token, impersonate="chrome131")


def _tempmail_message_key(message: dict) -> str:
...
def _combine_tempmail_message(message: dict) -> str:
    return "\n".join(
        [
...
def _extract_verification_codes_from_messages(messages, seen_messages=None):
    prioritized = []
    fallback = []
    seen_codes = set()
    ...
    for message in messages or []:
        ...
        message_key = _tempmail_message_key(message)
        if message_key in seen_messages:
            continue
        seen_messages.add(message_key)

        content = _combine_tempmail_message(message)
        if not content:
            continue
...
def _wait_for_tempmail_verification_code(fetch_inbox_func, mail_token: str, timeout: int = 120,
                                         poll_interval: int = 3, logger=None):
    deadline = time.time() + timeout
    seen_messages = set()

    while time.time() < deadline:
        inbox = fetch_inbox_func(mail_token)
        if inbox is None:
        ...
        if not isinstance(inbox, dict) or not inbox:
            if logger:
                logger("[OTP] 临时邮箱不可用或已过期")
            return None
        if inbox.get("expired") is True:
            if logger:
                logger("[OTP] 临时邮箱已过期")
            return None

        messages = inbox.get("emails") or []
        if isinstance(messages, list):
            codes = _extract_verification_codes_from_messages(messages, seen_messages)
            if codes:
                return codes[0]
        ...
        if logger:
            ...
            logger(f"[OTP] 等待中... ({elapsed}s/{timeout}s)")
        time.sleep(min(poll_interval, max(0, remaining)))

    if logger:
        logger(f"[OTP] 超时 ({timeout}s)")
    return None


def wait_for_verification_email(mail_token: str, timeout: int = 120):
    """等待并提取 OpenAI 验证码"""
    return _wait_for_tempmail_verification_code(
        _fetch_tempmail_lol_inbox,
        mail_token,
        timeout=timeout,
        poll_interval=3,
    )
...
class ChatGPTRegister:
    ...
    def _create_tempmail_lol_session(self):
        """创建 tempmail.lol 请求会话"""
        return _create_tempmail_lol_session(user_agent=self.ua, proxy=self.proxy)

    def create_temp_email(self):
        """创建 tempmail.lol 临时邮箱，返回 (email, mail_token)"""
        session = self._create_tempmail_lol_session()
        return _create_tempmail_lol_inbox(session, impersonate=self.impersonate)

    def _fetch_tempmail_lol_inbox(self, mail_token: str):
        """从 tempmail.lol 获取 inbox 原始响应"""
        session = self._create_tempmail_lol_session()
        return _request_tempmail_lol_inbox(session, mail_token, impersonate=self.impersonate)
    ...
    def wait_for_verification_email(self, mail_token: str, timeout: int = 120):
        ...
        code = _wait_for_tempmail_verification_code(
            self._fetch_tempmail_lol_inbox,
            mail_token,
            timeout=timeout,
            poll_interval=3,
            logger=self._print,
    ...
    def authorize_continue(self, email: str):
        """POST /api/accounts/authorize/continue — 推进 auth 状态机，建立 ChatGPT 客户端上下文"""
        ...
        self._log("3.5 Authorize Continue", "POST", url, r.status_code, data)
        ...
        if need_oauth_otp:
            ...
            while time.time() < otp_deadline and not otp_success:
                ...
                if isinstance(messages, list):
                    candidate_codes = [
                        code
                        for code in _extract_verification_codes_from_messages(messages, seen_otp_messages)
...
def _register_one(idx, total, proxy, output_file):
    """单个注册任务 (在线程中运行) - 使用 tempmail.lol 临时邮箱"""
    reg = None
    try:
        reg = ChatGPTRegister(proxy=proxy, tag=f"{idx}")

        # 1. 创建 tempmail.lol 临时邮箱
        reg._print("[Tempmail.lol] 创建临时邮箱...")
...
def run_batch(total_accounts: int = 3, output_file="registered_accounts.txt",
    ...
    """并发批量注册 - tempmail.lol 临时邮箱版"""

    actual_workers = min(max_workers, total_accounts)
    print(f"\n{'#'*60}")
    print(f"  ChatGPT 批量自动注册 (tempmail.lol 临时邮箱版)")
...
def main():
    print("=" * 60)
    print("  ChatGPT 批量自动注册工具 (tempmail.lol 临时邮箱版)")
    print("=" * 60)
